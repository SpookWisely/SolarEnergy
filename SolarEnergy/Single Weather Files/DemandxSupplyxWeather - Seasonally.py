...
So Just for reference sake I've attempted to go through with seasonal sequencing in the function under "create_seasonal_sequences" in the decision Tree function and
run into an issues with the sampling since I think at the moments it currently trying to compare the winter of the current year to the  
"""
...
def create_seasonal_sequences(df, feature_cols, target_cols, pad_to_max=True):
    ...
    X, y = [], []
    for year in years:
        # Get all seasons present for this year, in order
        seasons_in_year = [s for s in season_order if (year, s) in grouped.groups]
        for i in range(len(seasons_in_year) - 1):
            this_season = grouped.get_group((year, seasons_in_year[i]))
            next_season = grouped.get_group((year, seasons_in_year[i + 1]))

            x_seq = this_season[feature_cols].values
            if pad_to_max:
                max_len = max(len(grouped.get_group((year, s))[feature_cols]) for s in seasons_in_year[:-1])
                if x_seq.shape[0] < max_len:
                    pad_width = ((0, max_len - x_seq.shape[0]), (0, 0))
                    x_seq = np.pad(x_seq, pad_width, mode='constant')
            X.append(x_seq)
            y.append(next_season[target_cols].values[0])  # or use mean/aggregate as needed
...
def decisionTreeModelDS(mergedDs: pd.DataFrame):
    ...
    X_seq_flat = X_seq.reshape(X_seq.shape[0], -1)  # Flatten for sklearn models

    X_train, X_test, y_train, y_test = train_test_split(
        X_seq_flat, y_seq, test_size=0.3, shuffle=False
    )
    df = mergedDs.copy()
    df['TimeStamp'] = pd.to_datetime(df['TimeStamp'])
    df['year'] = df['TimeStamp'].dt.year
    ...
    cv_splits = min(3, n_samples) if n_samples > 1 else 2

    grid_search = GridSearchCV(base_model, param_grid, cv=cv_splits, scoring='neg_mean_squared_error', n_jobs=-1)
    ...
    for year in years:
        seasons_in_year = [s for s in season_order if (year, s) in grouped.groups]
        for i in range(len(seasons_in_year) - 1):
            transitions.append(f"{year} {seasons_in_year[i]}->{seasons_in_year[i+1]}")

    # Only keep the test set's transitions
    n_test = y_test.shape[0]
    x_labels = transitions[-n_test:]

    plt.figure(figsize=(10, 5))
    plt.plot(x_labels, y_demand_true[:, 0], label='Actual Demand', marker='o')
    plt.plot(x_labels, y_demand_pred[:, 0], label='Predicted Demand', marker='x')
    plt.title('Decision Tree: Actual vs Predicted Demand by Season Transition')
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(x_labels, y_demand_true[:, 1], label='Actual Supply', marker='o')
    plt.plot(x_labels, y_demand_pred[:, 1], label='Predicted Supply', marker='x')
    plt.title('Decision Tree: Actual vs Predicted Supply by Season Transition')
    plt.xlabel('Season Transition')
    plt.ylabel('MW')
    plt.xticks(rotation=45)
    plt.legend()
