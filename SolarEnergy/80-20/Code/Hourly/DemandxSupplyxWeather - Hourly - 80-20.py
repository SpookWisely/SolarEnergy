...
def gbModelDS(mergedDs: pd.DataFrame, plots:boolean):
    ...
    if plots == True:
        ...
        plt.plot(y_demand_true[:, 0], label='Actual Demand')
        plt.plot(y_demand_pred[:, 0], label='Predicted Demand')
        plt.title(f'Gradient Boosting Model: Actual vs Predicted Demand:  Hourly')
        plt.xlabel('Time')
        plt.ylabel('MW')
        plt.legend()
        plt.tight_layout()
        ...
        plt.plot(y_demand_true[:, 1], label='Actual Supply')
        plt.plot(y_demand_pred[:, 1], label='Predicted Supply')
        plt.title(f'Gradient Boosting Model: Actual vs Predicted Supply:  Hourly')
        plt.xlabel('Time')
        plt.ylabel('MW')
        plt.legend()
        plt.tight_layout()
...
def SVRModelDS(mergedDs: pd.DataFrame, plots:boolean):
    ...
    """
    Commented out SVR Feature importance for the sake of testing other parts first.
    # Initialize a dictionary to store SHAP values for each output
    shap_values_dict = {}
    def model_predict(X):
...
DecTree = ensure_real_result(decisionTreeModelDS(sp_FullMerg,True))
randForest = ensure_real_result(randomForestModelDS(sp_FullMerg,True))
xgb  = ensure_real_result(xgbModelDS(sp_FullMerg,True))
gb = ensure_real_result(gbModelDS(sp_FullMerg,True))
gbdt = ensure_real_result(GBDTModelDS(sp_FullMerg,True))
blstm = ensure_real_result(biDirectionalLSTMDS(sp_FullMerg,True))
lstm  = ensure_real_result(LSTMModelDS(sp_FullMerg,True))
gru  = ensure_real_result(GRUModelDS(sp_FullMerg,True))
svr  = ensure_real_result(SVRModelDS(sp_FullMerg,True))
mlp  = ensure_real_result(MLPModelDS(sp_FullMerg,True))
cnn  = ensure_real_result(CNNModelDS(sp_FullMerg,True))
nsga2cnn  = ensure_real_result(NSGA2_CNN_ModelDS(sp_FullMerg,True))
nsga3cnn  = ensure_real_result(NSGA3_CNN_ModelDS(sp_FullMerg,True))
...
print("\nModel Ranking Full Merged (Best to Worst) Hourly:")
